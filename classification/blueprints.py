...
class BluePrint(classification_models.ClassificationModels):
    ...
    def ml_bp04_train_test_binary_full_processing_blended_ensemble(self, df=None):
        ...
        if skip_train:
            ...
            self.xg_boost_train(autotune=False)
        self.xgboost_predict()
        self.smote_data()
        if skip_train:
            pass
            ...
            self.sklearn_ensemble_train()
        self.sklearn_ensemble_predict()
        algorithm = 'blended_ensemble'
        blended_probs = (self.predicted_probs["sklearn_ensemble"] + self.predicted_probs["xgboost"] + self.predicted_probs["lgbm"])/2
        if self.class_problem == 'binary':
            predicted = blended_probs > self.preprocess_decisions[f"probability_threshold"]
            predicted_classes = np.asarray([np.argmax(line) for line in predicted])
        else:
