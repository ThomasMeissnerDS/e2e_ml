...
class DeepQuantileRegression(
    ...
    def create_nn_train_dataset(self):
        ...
        train_dataset = RegressionDataset(
            torch.from_numpy(X_train.to_numpy(dtype="float32")).float(),
            torch.from_numpy(Y_train.to_numpy(dtype="float32")).float(),
        )
    ...
    def create_nn_test_dataset(self):
        ...
        test_dataset = RegressionDataset(
            torch.from_numpy(X_test.to_numpy(dtype="float32")).float(),
            torch.from_numpy(Y_test.to_numpy(dtype="float32")).float(),
        )
        return test_dataset
    ...
    def create_dual_nn_train_dataloader(self, train_batch_size=None, workers=None):
        if train_batch_size:
            pass
        else:
            train_batch_size = self.quantile_nn_settings["train_batch_size"]

        if workers:
            pass
        else:
            workers = self.quantile_nn_settings["num_workers"]
    ...
    def create_dual_nn_test_dataloader(self, test_batch_size=None, workers=None):
        if test_batch_size:
            pass
        else:
            test_batch_size = self.quantile_nn_settings["test_batch_size"]
    ...
    def pred_dual_nn_dataloader(self, pred_batch_size=None, workers=None):
        if pred_batch_size:
            pass
        else:
            pred_batch_size = self.quantile_nn_settings["pred_batch_size"]
    ...
    def get_dual_nn_architecture(self, type="ann", num_features=1, output_dim=1):
        if type == "ann":

            class MultipleRegression(nn.Module):
                def __init__(self, num_features, output_dim=output_dim):
                    super(MultipleRegression, self).__init__()

                    self.output_dim = output_dim
                    self.layer_1 = nn.Linear(num_features, 4096)
                    self.layer_2 = nn.Linear(4096, 512)
                    self.layer_3 = nn.Linear(512, 512)
                    self.layer_4 = nn.Linear(512, 4096)
                    self.layer_5 = nn.Linear(4096, 64)
                    self.layer_out = nn.Linear(64, self.output_dim)
            ...
            model = MultipleRegression(num_features=num_features, output_dim=3)
            return model
        else:
            ...
            model = SoftOrdering1DCNN(
                ...
                dropout_input=self.quantile_nn_settings["drop_out"],
                dropout_hidden=self.quantile_nn_settings["drop_out"],
                dropout_output=self.quantile_nn_settings["drop_out"],
                output_dim=3,
            )
            return model

    def dual_nn_model_setup(self, epochs=None):
        if self.prediction_mode:
            pass
        else:
            ...
            model = self.get_dual_nn_architecture(
                type=self.quantile_nn_settings["architecture"],
                num_features=self.preprocess_decisions["num_features"],
            )
            model.to(device)
            model.train()
            LR = self.quantile_nn_settings["learning_rate"]
            optimizer = torch.optim.Adam(
                model.parameters(),
                lr=LR,
                weight_decay=self.quantile_nn_settings["weight_decay"],
            )

            if epochs:
                pass
            else:
                epochs = self.quantile_nn_settings["epochs"]
            epochs = epochs
            train_steps = int(
                len(X_train) / self.quantile_nn_settings["train_batch_size"] * epochs
            )
    ...
    def nn_dual_training(self, train_dataloader, model, optimizer, scheduler):
        ...
        for X_train_batch, y_train_batch in train_dataloader:
            ...
            with torch.cuda.amp.autocast():
                ...
                loss = self.quantile_loss(
                    y_train_pred, y_train_batch, self.quantile_nn_settings["quantiles"]
                )
    ...
    def nn_dual_validating(self, valid_dataloader, model):
        ...
        for X_test_batch, y_test_batch in valid_dataloader:
            ...
            with torch.no_grad():
                ...
                loss = self.quantile_loss(
                    y_train_pred, y_test_batch, self.quantile_nn_settings["quantiles"]
                )
    ...
    def nn_dual_predicting(self, pred_dataloader, model, pathes):
    ...
    def deep_quantile_regression_train(self):
        if self.prediction_mode:
            pass
        else:
            ...
            train_dataloader = self.create_dual_nn_train_dataloader()
            test_dataloader = self.create_dual_nn_test_dataloader()
            (
                model,
                optimizer,
                train_steps,
            ...
            for epoch in tqdm(range(self.quantile_nn_settings["epochs"])):
                print("---------------" + str(epoch) + "start-------------")
                trainloss, trainscore = self.nn_dual_training(
                    train_dataloader, model, optimizer, scheduler
                )
                trainlosses.append(trainloss)
                trainscores.append(trainscore)
                print("trainscore is " + str(trainscore))
                preds, validloss, valscore = self.nn_dual_validating(
            ...
            for fold in range(self.quantile_nn_settings["nb_model_to_create"]):
                ...
                train_steps = int(
                    len(X_train)
                    / self.quantile_nn_settings["train_batch_size"]
                    * self.quantile_nn_settings["epochs"]
                )
                num_steps = int(train_steps * 0.1)
                scheduler = get_linear_schedule_with_warmup(
                    optimizer, num_steps, train_steps
    ...
    def deep_quantile_regression_predict(self):
        ...
        try:
            ...
            pass
        print(pthes)
        pred_dataloader = self.pred_dual_nn_dataloader()
        allpreds, mode_cols = self.nn_dual_predicting(pred_dataloader, model, pthes)

        if self.prediction_mode:
            self.dataframe["deep_quantile_regression_median"] = self.dataframe[
                mode_cols
            ].median(axis=1)
            self.dataframe["deep_quantile_regression_mean"] = self.dataframe[
                mode_cols
            ].mean(axis=1)
            self.dataframe[
                "deep_quantile_regression_mean"
            ] = self.target_skewness_handling(
                preds_to_reconvert=self.dataframe[
                    "deep_quantile_regression_mean"
                ].values,
            ...
            self.predicted_values["deep_quantile_regression"] = self.dataframe[
                "deep_quantile_regression_mean"
            ]
            ...
            X_test["deep_quantile_regression_median"] = X_test[mode_cols].median(axis=1)
            X_test["deep_quantile_regression_mean"] = X_test[mode_cols].mean(axis=1)
            X_test["deep_quantile_regression_mean"] = self.target_skewness_handling(
                preds_to_reconvert=X_test["deep_quantile_regression_mean"].values,
                mode="revert",
            )
