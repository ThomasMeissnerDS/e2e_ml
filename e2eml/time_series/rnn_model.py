...
class RNNModel(postprocessing.FullPipeline, TimeSeriesDataset):
    """
    ...
    def create_nn_train_dataset(self):
        ...
        train_dataset = TimeSeriesDataset(
            ...
            sequence_length=self.rnn_settings["seq_len"],
        )
    ...
    def create_nn_train_dataloader(self, train_batch_size=None, workers=None):
        if train_batch_size:
            ...
            train_batch_size = self.rnn_settings["train_batch_size"]

        if workers:
            pass
        else:
            workers = self.rnn_settings["num_workers"]
    ...
    def create_nn_test_dataloader(self, test_batch_size=None, workers=None):
        if test_batch_size:
            ...
            test_batch_size = self.rnn_settings["test_batch_size"]
    ...
    def pred_nn_dataloader(self, pred_batch_size=None, workers=None):
        if pred_batch_size:
            ...
            pred_batch_size = self.rnn_settings["pred_batch_size"]
    ...
    def loss_fn(self, output, target):
        if self.rnn_settings["regression_loss"] == "mse":
            return torch.sqrt(nn.MSELoss()(output, target))
        elif self.rnn_settings["regression_loss"] == "smoothl1":
            return torch.sqrt(nn.SmoothL1Loss()(output, target))
        elif self.rnn_settings["regression_loss"] == "l1":
            return torch.sqrt(nn.L1Loss()(output, target))
        elif self.rnn_settings["regression_loss"] == "poisson":
    ...
    def get_nn_architecture(
        ...
        model = RNNModel(input_dim, hidden_dim, layer_dim, output_dim, dropout_prob)
        return model
    ...
    def nn_model_setup(self, epochs=None):
        if self.prediction_mode:
            ...
            model = self.get_nn_architecture(
                ...
                hidden_dim=self.rnn_settings["hidden_dim"],
                layer_dim=self.rnn_settings["layer_dim"],
                output_dim=1,
                dropout_prob=self.rnn_settings["drop_out"],
            )
            model.to(device)
            model.train()
            LR = self.rnn_settings["learning_rate"]
            optimizer = torch.optim.Adam(
                model.parameters(),
                lr=LR,
                weight_decay=self.rnn_settings["weight_decay"],
            )

            if epochs:
                pass
            else:
                epochs = self.rnn_settings["epochs"]
            epochs = epochs
            train_steps = int(
                len(X_train) / self.rnn_settings["train_batch_size"] * epochs
            )
    ...
    def rnn_train(self):
        if self.prediction_mode:
            ...
            for epoch in tqdm(range(self.rnn_settings["epochs"])):
            ...
            for fold in range(self.rnn_settings["nb_model_to_create"]):
                ...
                train_steps = int(
                    len(X_train)
                    / self.rnn_settings["train_batch_size"]
                    * self.rnn_settings["epochs"]
                )
                num_steps = int(train_steps * 0.1)
                scheduler = get_linear_schedule_with_warmup(
                    optimizer, num_steps, train_steps
    ...
    def rnn_predict(self):
        ...
        if self.prediction_mode:
            self.dataframe["rnn_median"] = self.dataframe[mode_cols].median(axis=1)
            self.dataframe["rnn_mean"] = self.dataframe[mode_cols].mean(axis=1)

            # reverse scale target and df
            self.dataframe[self.target_variable] = self.dataframe["rnn_mean"]
            ...
            self.predicted_values["rnn"] = self.dataframe[self.target_variable]

        else:
            X_train, X_test, Y_train, Y_test = self.unpack_test_train_dict()

            if self.rnn_settings["keep_best_model_only"]:
            ...
            X_test["rnn_median"] = X_test[mode_cols].median(axis=1)
            X_test["rnn_mean"] = X_test[mode_cols].mean(axis=1)
            # reverse scale target and df
            X_test[self.target_variable] = X_test["rnn_mean"]
            self.scale_with_target(mode="reverse", drop_target=False)
            ...
            self.predicted_values["rnn"] = X_test[self.target_variable]
