    ...
    def get_best_slice(self, min_rows_considered=20):
        ...
        if mode in ["Categorical only", "Categorical and numerical"]:
            ...
            for col, val_list in (top_cat_slice_param.values[0]).items():  # noqa: B007
                for cat, value in (
                    top_cat_slice_param.values[0][col]
                ).items():  # noqa: B007
                    if value == 0:
    ...
    def slice_df_binary(self):  # noqa: C901
        def binary_objective(trial):  # this does not allow additional parameters!
            ...
            try:
                ...
                mae = temp_df[self.target_col].sum() / df_len
                self.append_results(param, numerical_param, mae, df_len, mode)
            except KeyError:
                mae = 999999999

            if self.supress_granular_prints:
                print(
                    f"""Trial {self.trial_counter} finished with score {mae} with df length of {df_len}."""
                )

            if self.trial_counter == 0 or mae < self.best_score:
                self.best_score = mae
...
def analyse_results(
    ...
    :param study_direction: Expects either 'maximize' or 'minimize'. According to this the routine
    will try to find segments with maximum or minimum error rates.
    ...
    holdout_df["abs_delta"] = np.abs(
        holdout_df["true_target"] - holdout_df["pred_target"]
    )

    study_maker_fl = AutoDataSlicer(
        holdout_df,
        target_col="abs_delta",
        objective="regression",
